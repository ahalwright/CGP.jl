...
# Test the version of pheno_evolve() that has num_circuits as an argument.
...
# Test the case where pheno_evolve() sometimes fails to find num_circuits_per_goal.
max_steps = 80
max_tries = 5
num_circuits_per_goal = 5
ph = [0x0011]
c_steps_list = pheno_evolve( p, funcs, ph, num_circuits_per_goal, max_tries, max_steps );
circuits_list = map(x->x[1], c_steps_list );
map( c->(@test output_values(c)==ph), circuits_list )  # Test that each evolved circuit maps to ph
