...
Random.seed!(1); 
ddf=run_random_walks_parallel(4,100,p,1000,output_dict=true) 
Random.seed!(1); 
mdf=run_random_walks_parallel(4,100,p,1000,output_dict=false) 
@assert ddf==mdf
...
@assert matrix_to_dict(dict_to_matrix(dd,ssize)) == dd
@assert dict_to_matrix(matrix_to_dict(M),ssize) == M
