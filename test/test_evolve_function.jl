...
function test_reduce_numactive( p::Parameters, numpairs::Int64, numtrials::Int64, maxsteps::Int64; csvfile::String="" )
  df = DataFrame()
  df.numinputs = Int64[]
  df.numgates = Int64[]
  ...
  rowlist = [ p.numinputs, p.numinteriors, p.numlevelsback, numpairs, numtrials, maxsteps ]
  for npair = 1:numpairs
    ...
    #nactive_lists = map(i->reduce_nactive_helper(c,maxsteps), collect(1:numtrials ) )
    nactive_lists = pmap(i->reduce_nactive_helper(c,maxsteps), collect(1:numtrials ) )
    nactive1 = vcat( [ na[1] for na in nactive_lists]... )
    nactive2 = vcat( [ na[2] for na in nactive_lists]... )
    nactive3 = vcat( [ na[3] for na in nactive_lists]... )
    ...
    row_results = [mean(nactive1),std(nactive1),mean(nactive2),std(nactive2),mean(nactive3),std(nactive3)]
