...
# See Figure 2a of Tononi Edelman and Sporns
function complexity8( c::Chromosome; base::Float64=2.0 )
  n = c.params.numinteriors
  numinputs = c.params.numinputs
  outputs = output_values(c)
  (IN, X, O) = node_values( c )  # lists of cached values of nodes, note letter O vs digit 0
...
# Does not use get_bits()
# See Figure 2a of Tononi Edelman and Sporns
function complexity8( X::Vector{MyInt}; base::Float64=2.0 )
  n = length(X)
  IX = integration( X, base=base )
  Xinds = collect(1:length(X)) 
  ssum = 0.0
  for k = 1:(n-1)
    subsets = [X[setdiff(Xinds,s)] for s in combinations(Xinds,k)]
    IXk = map( x->integration(x,base=base), subsets )
    avgIXk = sum(IXk)/length(IXk)
    ssum += k/n*IX - avgIXk
  end
  ssum
...
#=
# This function is meaningless because complexity5 already applies get_bits.  
# So applying get_bits twice is not a good idea
...
# Returns complexity of get_bits of the  node values of the interior nodes of c )
...
#=
# This function is meaningless because degeneracy already applies get_bits.  
# So applying get_bits twice is not a good idea
# x can be generated by randgoal( numinputs, 2^numinputs )
function gb_degeneracy( x::Vector{MyInt}, numinputs::Int64; base::Float64=2.0 )
  degeneracy( get_bits( x, numinputs ), base=base )
end

# Returns degeneracy of get_bits of the  node values of the interior nodes of c )
function gb_degeneracy_chrome( c::Chromosome; base::Float64=2.0 )
  output_values(c)
  gb_degeneracy( c, c.params.numinputs, base=base)
end
=#
