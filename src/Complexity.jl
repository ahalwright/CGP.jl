...
function run_k_complexity_mutate_all( p::Parameters, funcs::Vector{Func}, phlist::GoalList, numcircuits::Int64, max_tries::Int64, max_steps::Int64;
  ...
  result_list = map( ph->k_complexity_mutate_all( p, funcs, ph, numcircuits, max_tries, max_steps, k_dict,
      use_lincircuit=use_lincircuit, use_mut_evolve=use_mut_evolve, print_steps=print_steps ), phlist )
...
function kcomp_summary_dataframe( k_comp_rebased::DataFrame )
  ...
  ksummary = [ RebasedVector(i,zeros(Int64,17)) for i = 1:8 ]
  for i = 1:size(k_comp_rebased)[1]
    ...
    #ksummary[j] = kadd(ksummary[j],k_comp_rebased.rebased_vect[i].vect)
  end
  ksummary
end
